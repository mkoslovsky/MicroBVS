...
dm_lm_bvs_full_R <- function( iterations = 10000, thin = 10, y = NULL, z = NULL, x = NULL, alpha = NULL, phi = NULL,
                         psi = NULL, zeta = NULL, xi = NULL, beta = NULL, sigma2 = NULL, sigma2_alpha = sqrt( 10 ), sigma2_phi = sqrt( 10 ),
                         h_alpha = 1, h_beta = 1, a_m = 1, b_m = 9, a = 1, b = 9,
                         a_0 = 2, b_0 = 2, a_G = log(0.1/0.9), b_G = 0.5, Omega = NULL, G = NULL, v0 = 0.01, v1 = 10, pie = NULL, lambda = 1, prior = "BB", rate = 1, seed = 1 ){
  library(mvtnorm)
  library(MCMCpack)
  library(Rcpp)
  library(ggplot2)
  ...
  # beta - Covariates associated with the balances in the linear portion of the model
  # sigma2 - variance of the data in linear portion of the model
  ...
  # Beta for balances
  beta. <- matrix( 0, nrow = B_sim, ncol = samples )
  
  # Sigma2 for linear model
  ...
  beta.[ , 1] <- ifelse( is.null( beta ), 0, beta ) 
  sigma2[ 1 ] <- ifelse( is.null( sigma2 ), 1, sigma2 )
  ...
  # Run model
  output <- dm_lm_bvs_full( iterations, thin, alpha., y, z, x, phi., psi., cc, uu, beta., sigma2., sigma2_alpha, zeta., xi., sigma2_phi, a, b, a_0, b_0, h_alpha, h_beta, a_m, b_m,
                       prior,  Omega., G., Var., S., v0, v1, a_G, b_G, pie, lambda, rate )
