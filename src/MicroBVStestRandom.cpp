...
arma::mat matrixize( arma::vec x, int n_row, int n_col ){
  
  arma::mat matrix( n_row , n_col );
  matrix.zeros();
  
  for( int j = 0; j < n_col; ++j ){
    for( int i = 0; i < n_row; ++i ){
      matrix( i, j ) = x[ n_row*j + i ];
    }
  }
  return matrix;
}
...
// Make Zstar from Z, nu, L (DOUBLE CHECK)  
arma::vec make_Zstar( arma::mat Z, arma::mat nu, arma::mat L, arma::vec subject, int j,  int d ) { 
  int obs = Z.n_rows;
  int D = Z.n_cols;
  int K = L.n_cols;
  arma::vec Zstar( obs );
  ...
  arma::mat L_j = help::matrixize( L.col( j ), D, D );
  
  for( int i = 0; i < obs ; ++i ){
    int sub = subject[ i ];
      double sum_gamma_nu = 0;
      for( int l = 0; l < d  ; ++l ){
        int nu_col_l = D*j + l;
        sum_gamma_nu += L_j( d, l ) * nu( sub, nu_col_l ); 
        
        int nu_col_d = D*j + d;
        Zstar[ i ] = Z( i, d ) * ( nu( sub, nu_col_d ) + sum_gamma_nu );
    }
  }
...
// Make Zhat from Z, kappa, mu
arma::mat make_Zhat( arma::mat Z, arma::mat kappa, arma::mat nu) {
  int D = Z.n_cols;
  int obs = Z.n_rows;
  ...
  for( int m = 0; m < D - 1; ++m ){
    for( int l = m + 1; l < D; ++l ){
      Zhat.col( count ) = kappa( j, l ) * Z.col( l ) % nu.col( m );
      count += 1;
    }
  }
  return Zhat; 
...
// Function :: Between Step (jointly update kappa and xi) with add/delete using BB prior (DOUBLE CHECK)
List between_kappa_xi_augment_update_BB_cpp(
    arma::mat loggamma, 
    arma::mat cc,
    arma::mat z,
    arma::mat kappa,
  ...
  for( int d = 1; d < D; ++d ){
    ...
    if( kappa( j, d ) == 1 ){
      ...
      double kappa_prop_raw = m_star + sqrt( v_star )*rnorm( 1 )[ 0 ];
      kappa_prop =  std::abs( kappa_prop_raw ); 
      xi_RE_prop = 1;
    }
    ...
    arma::vec Z_star = make_Zstar( z, nu, L, subject, j, d );
    
    for( int n = 0 ; n < N ; ++n ){
      loggamma_prop[ n ] = loggamma( n, j ) - kappa( j, d ) * Z_star[ n ] + kappa_prop * Z_star[ n ];
    }
    ...
    // Calculate r given an add or delete step 
    if( xi_RE( j, d ) == 1 ){
      r = log_like_prop + help::log_zeta_pj_cpp( 0, a, b) + help::log_fold_normal_cpp( kappa( j, d ), m_star, v_star ) - ( log_like + help::log_fold_normal_cpp( kappa( j, d ), m_0, v_0 ) + help::log_zeta_pj_cpp( 1, a, b ) );
    }else{
      r = log_like_prop + help::log_zeta_pj_cpp( 1, a, b) + help::log_fold_normal_cpp( kappa_prop, m_0, v_0 ) - ( log_like + help::log_zeta_pj_cpp( 0, a, b ) + help::log_fold_normal_cpp( kappa_prop, m_star, v_star ) );
    }
    ...
    // Determine acceptance
    if( aa < r ){
      xi_RE( j, d ) = xi_RE_prop;
      kappa( j, d ) = kappa_prop;
      loggamma.col( j ) = loggamma_prop;
    }
...
// Function :: Within Step ( update kappa ) (DOUBLE CHECK )
List within_kappa_augment_cpp(
    arma::mat loggamma, 
    arma::mat cc,
    arma::mat z,
    arma::mat kappa,
  ...
  for( int j = 0; j < J; ++j ){
    for( int d = 1; d < D; ++d ){
      if( kappa( j , d ) == 1 ){
        ...
        // Calculate ratio
        double r = log_like_prop + help::log_fold_normal_cpp( kappa_prop, m_0, v_0 ) + help::log_fold_normal_cpp( kappa( j, d ), m_star, v_star ) - ( log_like + help::log_fold_normal_cpp( kappa( j, d ), m_0, v_0 ) + help::log_fold_normal_cpp( kappa_prop, m_star, v_star ) ) ;
  ...
  List kappa_out( 2 );
  kappa_out[ 0 ] = kappa;
  kappa_out[ 1 ] = loggamma;
  return kappa_out;
}

// Function :: Update L (DOUBLE CHECK )
List update_L_cpp(
    arma::mat loggamma, 
    arma::mat cc,
    arma::mat z,
  ...
  for( int j = 0; j < J; ++j ){
    ...
    for( int d = 0; d < D; ++d ){
      ...
      // Update the appropriate columns and rows of L_j
      for( int m = 0; m < d; ++m ){
      ...
      for( int m = d + 1; m < D; ++m ){
        ...
        ell_prop = m_star + sqrt( v_star )*rnorm( 1 )[ 0 ];
